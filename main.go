...
var (
	...
	urlList		  map[string]*Promise = make(map[string]*Promise)
	curIndex      int
	curListVideos []Video
...
func NewPromise(f func() string) *Promise {
    p := &Promise{}
    p.wg.Add(1)
    go func() {
        p.res = f()
...
func (p *Promise) Then(r func(string)) {
	p.wg.Wait()
	r(p.res)
}
...
func genericCmd(index int64, cmd string, paramsList []interface{}) {
	...
	case "setPlaylist":
		...
		if data["currentIndex"] != nil {
			index, err := strconv.Atoi(data["currentIndex"].(string))
			if err == nil {
				curIndex = index
				curVideo = curListVideos[index]
			}
		}
		// set startTime:
		...
		for _, video1 := range curList {
			video := video1
			if(urlList[video] == nil){
				urlList[video] = NewPromise(func() string{
					args := []string{"-g", "-f", "mp4", "https://youtube.com?v="+video}
		...
		curUrl := ""
		urlList[curVideo.Id].Then(func(urlLink string){
			curUrl = urlLink
			
		})
		videoDuration = curVideo.Length
		if(!timeTrackerRunning){ 
			go trackTime()
			timeTrackerRunning = true
		}
		msgPrintln(fmt.Sprint("video_id ", curUrl))
		postBind("nowPlaying", map[string]string{
			"videoId":      curVideoId,
	...
	case "next":
		msgPrintln("next")
		if curIndex+1 < len(curListVideos) {
			curIndex++
			curTime = 0
			startTime = time.Now()
			...
			curUrl := ""
			urlList[curVideoId].Then(func(urlLink string){
				curUrl = urlLink
			})
			videoDuration = curVideo.Length
			msgPrintln(fmt.Sprint("video_id ", curUrl))
